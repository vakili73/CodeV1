...
class Siamese(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        anchor_gen = ImageDataGenerator(
            rotation_range=20,
            width_shift_range=0.2,
            height_shift_range=0.2,
            horizontal_flip=True)
        anchor_gen.fit(self.x)
        other_gen = ImageDataGenerator(
            rotation_range=20,
            width_shift_range=0.2,
            height_shift_range=0.2,
            horizontal_flip=True)
        ...
        for (anchor_x, _), (other_x, _) in zip(anchor_generator, other_generator):
            batch.append((anchor_x, other_x, np.zeros((self.batch_size//2))))
            break
        for (anchor_x1, _), (anchor_x2, _) in zip(anchor_generator, anchor_generator):
            batch.append((anchor_x1, anchor_x2, np.ones((self.batch_size//2))))
            break
...
datagen = Siamese(x_train, y_train, num_classes)
