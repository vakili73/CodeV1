...
def calculate_clusters_mxzero(b, X_train, X_test, max_iter=1, n_init=1 ,n_jobs=1):
    kmeans = KMeans(n_clusters=b, random_state=0, max_iter=max_iter, n_init=n_init, n_jobs=n_jobs)
    kmeans.fit(X_train)
...
def run_cknn(X_train, Y_train, X_test, Y_test, train_clusters, test_clusters, k, weights='uniform', n_jobs=1):
    ...
    for c in range(n_clusters):
        ...
        if X_train_cluster.shape[0] > 0 and X_test_cluster.shape[0] > 0:
            ...
            knnbc = KNeighborsClassifier(weights=weights,
                n_neighbors=k_value, algorithm='auto', n_jobs=n_jobs)
        ...
        for data_name, schemas in all_data.items():
            for schema, data in schemas.items():
                ...
                for k in (1, 3, 5, 7, 9):
                    ...
                    # Run experiment ckNN
                    result_1 = run_cknn(
                        X_train, Y_train, X_test, Y_test, train_clusters, test_clusters, k, 'uniform', njobs)
                ...
                # KMeans max
                train_clusters, test_clusters, initial_time = calculate_clusters_mxzero(
                    b, X_train, X_test, 1, 10, njobs)
                ...
                for k in (1, 3, 5, 7, 9):
                    ...
                    print_result(data_name+'_'+schema, '10ckNN', 0, b,
                                 k, 0, X_train, X_test, initial_time, result_1)
                ...
                for k in (1, 3, 5, 7, 9):
                    ...
                    print_result(data_name+'_'+schema, '1ckNN', 0, b,
                                 k, 0, X_train, X_test, initial_time, result_1)
                ...
                for k in (1, 3, 5, 7, 9):
                    ...
                    # Run experiment ckNN
                    result_1 = run_cknn(
                        X_train, Y_train, X_test, Y_test, train_clusters, test_clusters, k, 'distance',  njobs)

                    print_result(data_name+'_'+schema, 'w10ckNN', 0, b,
                                 k, 0, X_train, X_test, initial_time, result_1)
                ...
                for k in (1, 3, 5, 7, 9):
                    ...
                    print_result(data_name+'_'+schema, 'w1ckNN', 0, b,
                                 k, 0, X_train, X_test, initial_time, result_1)
