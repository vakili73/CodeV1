...
def model_generator(model_schema, schema, nb_classes, input_shape, model_version):
    ...
    for item in schema:
        ...
        for layer, attr in model_schema[item]:
            attr = attr.copy()
            if 'units' in attr and (attr['units'] == None or attr['units'] == 'None'):
                attr['units'] = nb_classes
            elif 'units' in attr and isinstance(attr['units'], str) and attr['units'].startswith('/'):
                attr['units'] = int(nb_classes/float(attr['units'][1:]))
            elif 'units' in attr:
                attr['units'] = int(attr['units'])
