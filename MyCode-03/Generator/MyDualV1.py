    ...
    def __getitem__(self, idx):
        ...
        for _ in range(self.batch_size//2):
            ...
            batch.append((self.x[self.indices[other][o_index]],
                          ...
                          to_categorical(anchor, self.n_cls), 0.))
            ...
            batch.append((self.x[self.indices[anchor][a_index[0]]],
                          ...
                          to_categorical(anchor, self.n_cls), 1.))
        in_1, in_2, out_1, out_2, out = zip(*batch)
        return [np.stack(in_1), np.stack(in_2)], np.c_[np.concatenate([
            np.stack(out_1), np.stack(out_2)], axis=-1), np.stack(out)]
